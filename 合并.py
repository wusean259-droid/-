...
# ⚙️ 1. 全局唯一配置 (系统的“锚”)
# ==========================================
st.set_page_config(page_title="综合量化投研中枢", page_icon="📈", layout="wide")
...
def run_fof_dashboard():
    ...
    if bench_df is None:
        st.sidebar.error("⚠️ 沪深300大盘基准获取失败（网络被拦截）。Tab 4 的 Alpha 计算将受限。")
    else:
    ...
    if st.sidebar.checkbox("🚀 开启量化引擎", value=False):
        ...
        tab1, tab2, tab3, tab4, tab5 = st.tabs(["🧩 单体净值与持仓透视", "🤝 组合相关性矩阵", "⚖️ 宏观板块对冲雷达", "🧠 经理真实任期归因", "🏆 投资组合整体回测"])
        ...
        # TAB 4: 基金经理任期归因
        with tab4:
            st.subheader("🧠 剥离时代红利，拷问基金经理真实 Alpha")
            results = []
            for code in codes:
                ...
                results.append({
                    ...
                    "📈 同期大盘(Beta)": f"{bench_ret*100:.2f}%" if bench_ret is not None else "数据缺失",
                    "🔥 超额能力(Alpha)": f"{alpha*100:.2f}%" if alpha is not None else "数据缺失"
                })
                
            if results:
                st.dataframe(pd.DataFrame(results), use_container_width=True, hide_index=True)
                st.success("✅ **报告解读**：关注【超额能力(Alpha)】一栏。如果 Alpha 是负数，说明该经理这段时间的超高收益全是市场行情（Beta）给的，他甚至还在跑输大盘！")
            else:
